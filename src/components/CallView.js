...
const CallView = props => {
  ...
  const toggleMicrophone = _ => {
    for (const stream of call.stream.getAudioTracks()) {
  ...
  const _renderCall = () => {
    if (!call.incomingCall) return <div />
    ...
    return (
      <>
        ...
        <DialogActions style={{ backgroundColor: 'black', display: 'flex', justifyContent: 'center' }}>
          <div
            onClick={_ => dispatch(acceptCall(call.incomingCall, call.incomingCall.SignalData))} style={{
              display: 'flex',
          ...
          <div
            onClick={() => dispatch(declineCall())} style={{
              display: 'flex',
              justifyContent: 'center',
              alignItems: 'center',
              bottom: 10,
      ...
      >
        ...
        <DialogContent style={{ backgroundColor: 'black', overflow: 'none' }}>
          <Draggable defaultPosition={{ x: 10, y: 10 }}>
            <video
              id='localVideo' draggable muted autoPlay playsInline controls={false} style={{
                width: '25%',
                minWidth: 100,
                maxWidth: 450,
                position: 'absolute',
        ...
        <DialogActions style={{ backgroundColor: 'black', display: 'flex', justifyContent: 'center' }}>
          <div
            onClick={toggleMicrophone} style={{
              display: 'flex',
              justifyContent: 'center',
              alignItems: 'center',
              bottom: 10,
